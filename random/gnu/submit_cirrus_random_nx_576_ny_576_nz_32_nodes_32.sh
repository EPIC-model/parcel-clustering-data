...
for i in $(seq 1 10); do
    srun --kill-on-bad-exit \
         ...
         --shuffle \
         --csvfname "cirrus-gnu-shmem-random-nx-576-ny-576-nz-32-nodes-32" \
         --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --kill-on-bad-exit \
	     --nodes=32 \
             ...
             --small-parcel-fraction 0.5 \
             --shuffle \
             --csvfname "cirrus-gnu-$g-random-nx-576-ny-576-nz-32-nodes-32" \
             --comm-type "$g"
        ...
        if test "false" = "true"; then
            srun --kill-on-bad-exit \
	         --nodes=32 \
                 ...
                 --small-parcel-fraction 0.5 \
                 --shuffle \
                 --csvfname "cirrus-gnu-$g-random-nx-576-ny-576-nz-32-nodes-32-subcomm" \
                 --comm-type "$g" \
                 --subcomm
        fi
    done
