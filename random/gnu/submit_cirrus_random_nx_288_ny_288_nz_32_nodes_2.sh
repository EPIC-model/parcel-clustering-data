...
for i in $(seq 1 10); do
    srun --kill-on-bad-exit \
         ...
         --shuffle \
         --csvfname "cirrus-gnu-shmem-random-nx-288-ny-288-nz-32-nodes-2" \
         --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --kill-on-bad-exit \
	     --nodes=2 \
             ...
             --small-parcel-fraction 0.5 \
             --shuffle \
             --csvfname "cirrus-gnu-$g-random-nx-288-ny-288-nz-32-nodes-2" \
             --comm-type "$g"
        ...
        if test "false" = "true"; then
            srun --kill-on-bad-exit \
	         --nodes=2 \
                 ...
                 --small-parcel-fraction 0.5 \
                 --shuffle \
                 --csvfname "cirrus-gnu-$g-random-nx-288-ny-288-nz-32-nodes-2-subcomm" \
                 --comm-type "$g" \
                 --subcomm
        fi
    done
