...
for i in $(seq 1 5); do
    srun --kill-on-bad-exit \
         ...
         --nfiles 10 \
         --size-factor 1.5 \
         --csvfname "cirrus-gnu-shmem-read-late-nx-128-ny-128-nz-128-nodes-2" \
         --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --kill-on-bad-exit \
             --nodes=2 \
             ...
             --nfiles 10 \
             --size-factor 1.5 \
             --csvfname "cirrus-gnu-$g-read-late-nx-128-ny-128-nz-128-nodes-2" \
             --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --kill-on-bad-exit \
                 ...
                 --nfiles 10 \
                 --size-factor 1.5 \
                 --csvfname "cirrus-gnu-$g-read-late-nx-128-ny-128-nz-128-nodes-2-subcomm" \
                 --comm-type "$g" \
                 --subcomm
        fi
    done
