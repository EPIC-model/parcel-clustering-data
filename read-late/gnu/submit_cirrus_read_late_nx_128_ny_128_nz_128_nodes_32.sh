...
for i in $(seq 1 5); do
    srun --kill-on-bad-exit \
         ...
         /tmp/benchmark_read \
         --dirname /work/e710/e710/mf248/parcel-clustering-data/rayleigh_taylor/rt-128x128x128/late-time \
         --ncbasename epic_rt_128x128x128_late \
         --niter 100 \
         --offset 1 \
         --nfiles 10 \
         --size-factor 1.5 \
         --csvfname "cirrus-gnu-shmem-read-late-nx-128-ny-128-nz-128-nodes-32" \
         --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --kill-on-bad-exit \
             --nodes=32 \
             ...
             --nfiles 10 \
             --size-factor 1.5 \
             --csvfname "cirrus-gnu-$g-read-late-nx-128-ny-128-nz-128-nodes-32" \
             --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --kill-on-bad-exit \
                 ...
                 --nfiles 10 \
                 --size-factor 1.5 \
                 --csvfname "cirrus-gnu-$g-read-late-nx-128-ny-128-nz-128-nodes-32-subcomm" \
                 --comm-type "$g" \
                 --subcomm
        fi
    done
