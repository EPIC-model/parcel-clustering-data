...
for i in $(seq 1 5); do
    srun --kill-on-bad-exit \
         ...
         --dirname /work/e710/e710/mf248/parcel-clustering-data/rayleigh_taylor/rt-64x64x64/late-time \
         --ncbasename epic_rt_64x64x64_late \
         --niter 100 \
         --offset 1 \
         --nfiles 10 \
         --size-factor 1.5 \
         --csvfname "cirrus-gnu-shmem-read-late-nx-64-ny-64-nz-64-nodes-1" \
         --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --kill-on-bad-exit \
             --nodes=1 \
             ...
             --nfiles 10 \
             --size-factor 1.5 \
             --csvfname "cirrus-gnu-$g-read-late-nx-64-ny-64-nz-64-nodes-1" \
             --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --kill-on-bad-exit \
                 ...
                 --nfiles 10 \
                 --size-factor 1.5 \
                 --csvfname "cirrus-gnu-$g-read-late-nx-64-ny-64-nz-64-nodes-1-subcomm" \
                 --comm-type "$g" \
                 --subcomm
        fi
    done
