...
for i in $(seq 1 5); do
    srun --nodes=128 \
        ...
        --nfiles 10 \
        --size-factor 4.0 \
        --csvfname "hotlum-cray-shmem-read-late-nx-256-ny-256-nz-256-nodes-128" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=128 \
            --ntasks=8192 \
            ...
            --nfiles 10 \
            --size-factor 4.0 \
            --csvfname "hotlum-cray-$g-read-late-nx-256-ny-256-nz-256-nodes-128" \
            --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --nodes=128 \
                ...
                --nfiles 10 \
                --size-factor 4.0 \
                --csvfname "hotlum-cray-$g-read-late-nx-256-ny-256-nz-256-nodes-128-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
