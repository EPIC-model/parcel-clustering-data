...
for i in $(seq 1 5); do
    srun --nodes=32 \
        ...
        --dirname /lus/bnchlu1/shanks/EPIC/data/rt-128x128x128/late-time \
        --ncbasename epic_rt_128x128x128_late \
        --niter 100 \
        --offset 1 \
        --nfiles 10 \
        --size-factor 1.5 \
        --csvfname "hotlum-cray-shmem-read-late-nx-128-ny-128-nz-128-nodes-32" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=32 \
            --ntasks=2048 \
            ...
            --nfiles 10 \
            --size-factor 1.5 \
            --csvfname "hotlum-cray-$g-read-late-nx-128-ny-128-nz-128-nodes-32" \
            --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --nodes=32 \
                ...
                --nfiles 10 \
                --size-factor 1.5 \
                --csvfname "hotlum-cray-$g-read-late-nx-128-ny-128-nz-128-nodes-32-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
