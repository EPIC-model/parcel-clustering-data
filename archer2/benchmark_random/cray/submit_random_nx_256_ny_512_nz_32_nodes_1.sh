...
for i in $(seq 1 10); do
    srun --nodes=1 \
        ...
        --shuffle \
        --csvfname "archer2-cray-shmem-random-nx-256-ny-512-nz-32-nodes-1" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=1 \
            --ntasks=128 \
            ...
            --small-parcel-fraction 0.5 \
            --shuffle \
            --csvfname "archer2-cray-$g-random-nx-256-ny-512-nz-32-nodes-1" \
            --comm-type "$g"
        ...
        if test "false" = "true"; then
            srun --nodes=1 \
                ...
                --small-parcel-fraction 0.5 \
                --shuffle \
                --csvfname "archer2-cray-$g-random-nx-256-ny-512-nz-32-nodes-1-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
