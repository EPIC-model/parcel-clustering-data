...
for i in $(seq 1 10); do
    srun --nodes=8 \
        ...
        --shuffle \
        --csvfname "archer2-gnu-shmem-random-nx-256-ny-512-nz-32-nodes-8" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=8 \
            --ntasks=1024 \
            ...
            --small-parcel-fraction 0.5 \
            --shuffle \
            --csvfname "archer2-gnu-$g-random-nx-256-ny-512-nz-32-nodes-8" \
            --comm-type "$g"
        ...
        if test "false" = "true"; then
            srun --nodes=8 \
                ...
                --small-parcel-fraction 0.5 \
                --shuffle \
                --csvfname "archer2-gnu-$g-random-nx-256-ny-512-nz-32-nodes-8-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
