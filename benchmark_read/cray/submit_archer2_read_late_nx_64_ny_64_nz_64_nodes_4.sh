...
for i in $(seq 1 10); do
    srun --nodes=4 \
        ...
        /tmp/benchmark_read \
        --dirname /work/e710/e710/mf248/parcel-clustering/scripts/rayleigh_taylor/rt-64x64x64/late-time \
        --ncbasename epic_rt_64x64x64_late \
        --niter 10 \
        --offset 1 \
        --nfiles 10 \
        --size-factor 1.5 \
        --csvfname "archer2-cray-shmem-read-late-nx-64-ny-64-nz-64-nodes-4" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=4 \
            --ntasks=512 \
            ...
            --nfiles 10 \
            --size-factor 1.5 \
            --csvfname "archer2-cray-$g-read-late-nx-64-ny-64-nz-64-nodes-4" \
            --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --nodes=4 \
                ...
                --nfiles 10 \
                --size-factor 1.5 \
                --csvfname "archer2-cray-$g-read-late-nx-64-ny-64-nz-64-nodes-4-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
