...
for i in $(seq 1 10); do
    srun --nodes=32 \
        ...
        --nfiles 10 \
        --size-factor 2.0 \
        --csvfname "archer2-cray-shmem-read-late-nx-256-ny-256-nz-256-nodes-32" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=32 \
            --ntasks=4096 \
            ...
            --nfiles 10 \
            --size-factor 2.0 \
            --csvfname "archer2-cray-$g-read-late-nx-256-ny-256-nz-256-nodes-32" \
            --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --nodes=32 \
                ...
                --nfiles 10 \
                --size-factor 2.0 \
                --csvfname "archer2-cray-$g-read-late-nx-256-ny-256-nz-256-nodes-32-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
