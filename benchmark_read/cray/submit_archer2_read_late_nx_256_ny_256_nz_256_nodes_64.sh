...
for i in $(seq 1 10); do
    srun --nodes=64 \
        ...
        /tmp/benchmark_read \
        --dirname /work/e710/e710/mf248/parcel-clustering/scripts/rayleigh_taylor/rt-256x256x256/late-time \
        --ncbasename epic_rt_256x256x256_late \
        --niter 10 \
        --offset 1 \
        --nfiles 10 \
        --size-factor 3.0 \
        --csvfname "archer2-cray-shmem-read-late-nx-256-ny-256-nz-256-nodes-64" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=64 \
            --ntasks=8192 \
            ...
            --nfiles 10 \
            --size-factor 3.0 \
            --csvfname "archer2-cray-$g-read-late-nx-256-ny-256-nz-256-nodes-64" \
            --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --nodes=64 \
                ...
                --nfiles 10 \
                --size-factor 3.0 \
                --csvfname "archer2-cray-$g-read-late-nx-256-ny-256-nz-256-nodes-64-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
