#!/bin/bash
#SBATCH --job-name=archer2-cray-read
#SBATCH --output=%x.o%j
...
for i in $(seq 1 10); do
    srun --nodes=32 \
        ...
        /tmp/benchmark_read \
        --dirname /work/e710/e710/mf248/parcel-clustering/scripts/rayleigh_taylor/rt-256x256x256/early-time \
        --ncbasename epic_rt_256x256x256_early \
        --niter 10 \
        --offset 1 \
        --nfiles 10 \
        --size-factor 2.0 \
        --csvfname "archer2-cray-shmem-read-early-nx-256-ny-256-nz-256-nodes-32" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=32 \
            --ntasks=4096 \
            ...
            --nfiles 10 \
            --size-factor 2.0 \
            --csvfname "archer2-cray-$g-read-early-nx-256-ny-256-nz-256-nodes-32" \
            --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --nodes=32 \
                ...
                --nfiles 10 \
                --size-factor 2.0 \
                --csvfname "archer2-cray-$g-read-early-nx-256-ny-256-nz-256-nodes-32-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
