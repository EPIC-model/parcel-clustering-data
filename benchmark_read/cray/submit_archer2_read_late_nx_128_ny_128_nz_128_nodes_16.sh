...
echo "Running on $SLURM_N16 nodes with $SLURM_NTASKS tasks."

module list

bin_dir=/work/e710/e710/mf248/cray/bin
PATH=${bin_dir}:$PATH
...
for i in $(seq 1 10); do
    srun --nodes=16 \
        ...
        /tmp/benchmark_read \
        --dirname /work/e710/e710/mf248/parcel-clustering/scripts/rayleigh_taylor/rt-128x128x128/late-time \
        --ncbasename epic_rt_128x128x128_late \
        --niter 10 \
        --offset 1 \
        --nfiles 10 \
        --size-factor 1.5 \
        --csvfname "archer2-cray-shmem-read-late-nx-128-ny-128-nz-128-nodes-16" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=16 \
            --ntasks=2048 \
            ...
            --nfiles 10 \
            --size-factor 1.5 \
            --csvfname "archer2-cray-$g-read-late-nx-128-ny-128-nz-128-nodes-16" \
            --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --nodes=16 \
                ...
                --nfiles 10 \
                --size-factor 1.5 \
                --csvfname "archer2-cray-$g-read-late-nx-128-ny-128-nz-128-nodes-16-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
