...
echo "Running on $SLURM_N32 nodes with $SLURM_NTASKS tasks."
...
for i in $(seq 1 10); do
    srun --nodes=32 \
        ...
        --nfiles 10 \
        --size-factor 1.5 \
        --csvfname "archer2-cray-shmem-read-late-nx-128-ny-128-nz-128-nodes-32" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=32 \
            --ntasks=4096 \
            ...
            --nfiles 10 \
            --size-factor 1.5 \
            --csvfname "archer2-cray-$g-read-late-nx-128-ny-128-nz-128-nodes-32" \
            --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --nodes=32 \
                ...
                --nfiles 10 \
                --size-factor 1.5 \
                --csvfname "archer2-cray-$g-read-late-nx-128-ny-128-nz-128-nodes-32-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
