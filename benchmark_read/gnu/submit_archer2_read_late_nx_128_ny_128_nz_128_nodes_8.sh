...
for i in $(seq 1 10); do
    srun --nodes=8 \
        ...
        --nfiles 10 \
        --size-factor 1.5 \
        --csvfname "archer2-gnu-shmem-read-late-nx-128-ny-128-nz-128-nodes-8" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=8 \
            --ntasks=1024 \
            ...
            --nfiles 10 \
            --size-factor 1.5 \
            --csvfname "archer2-gnu-$g-read-late-nx-128-ny-128-nz-128-nodes-8" \
            --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --nodes=8 \
                ...
                --nfiles 10 \
                --size-factor 1.5 \
                --csvfname "archer2-gnu-$g-read-late-nx-128-ny-128-nz-128-nodes-8-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
