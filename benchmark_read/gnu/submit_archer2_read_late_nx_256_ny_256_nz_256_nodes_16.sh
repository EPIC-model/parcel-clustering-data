...
for i in $(seq 1 10); do
    srun --nodes=16 \
        ...
        --nfiles 10 \
        --size-factor 1.5 \
        --csvfname "archer2-gnu-shmem-read-late-nx-256-ny-256-nz-256-nodes-16" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=16 \
            --ntasks=2048 \
            ...
            --nfiles 10 \
            --size-factor 1.5 \
            --csvfname "archer2-gnu-$g-read-late-nx-256-ny-256-nz-256-nodes-16" \
            --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --nodes=16 \
                ...
                --nfiles 10 \
                --size-factor 1.5 \
                --csvfname "archer2-gnu-$g-read-late-nx-256-ny-256-nz-256-nodes-16-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
