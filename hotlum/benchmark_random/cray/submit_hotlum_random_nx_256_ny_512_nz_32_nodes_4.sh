...
for i in $(seq 1 10); do
	...
	--hint=nomultithread \
        ...
        --shuffle \
        --csvfname "hotlum-cray-shmem-random-nx-256-ny-512-nz-32-nodes-4" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=4 \
            --ntasks=256 \
            ...
            --small-parcel-fraction 0.5 \
            --shuffle \
            --csvfname "hotlum-cray-$g-random-nx-256-ny-512-nz-32-nodes-4" \
            --comm-type "$g"
        ...
        if test "false" = "true"; then
            srun --nodes=4 \
                ...
                --small-parcel-fraction 0.5 \
                --shuffle \
                --csvfname "hotlum-cray-$g-random-nx-256-ny-512-nz-32-nodes-4-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
