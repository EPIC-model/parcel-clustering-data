...
for i in $(seq 1 5); do
    srun --nodes=64 \
        ...
        --nfiles 10 \
        --size-factor 3.0 \
        --csvfname "hotlum-cray-shmem-read-early-nx-256-ny-256-nz-256-nodes-64" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=64 \
            --ntasks=4096 \
            ...
            --nfiles 10 \
            --size-factor 3.0 \
            --csvfname "hotlum-cray-$g-read-early-nx-256-ny-256-nz-256-nodes-64" \
            --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --nodes=64 \
                ...
                --nfiles 10 \
                --size-factor 3.0 \
                --csvfname "hotlum-cray-$g-read-early-nx-256-ny-256-nz-256-nodes-64-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
