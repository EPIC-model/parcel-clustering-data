...
for i in $(seq 1 5); do
    srun --nodes=4 \
        ...
        --nfiles 10 \
        --size-factor 1.5 \
        --csvfname "hotlum-cray-shmem-read-early-nx-128-ny-128-nz-128-nodes-4" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=4 \
            --ntasks=256 \
            ...
            --nfiles 10 \
            --size-factor 1.5 \
            --csvfname "hotlum-cray-$g-read-early-nx-128-ny-128-nz-128-nodes-4" \
            --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --nodes=4 \
                ...
                --nfiles 10 \
                --size-factor 1.5 \
                --csvfname "hotlum-cray-$g-read-early-nx-128-ny-128-nz-128-nodes-4-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
