...
for i in $(seq 1 10); do
    srun --nodes=4 \
        ...
        --dirname /lus/bnchlu1/shanks/EPIC/data/rt-64x64x64/early-time \
        --ncbasename epic_rt_64x64x64_early \
        --niter 10 \
        --offset 1 \
        --nfiles 10 \
        --size-factor 1.5 \
        --csvfname "hotlum-cray-shmem-read-early-nx-64-ny-64-nz-64-nodes-4" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=4 \
            --ntasks=256 \
            ...
            --nfiles 10 \
            --size-factor 1.5 \
            --csvfname "hotlum-cray-$g-read-early-nx-64-ny-64-nz-64-nodes-4" \
            --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --nodes=4 \
                ...
                --nfiles 10 \
                --size-factor 1.5 \
                --csvfname "hotlum-cray-$g-read-early-nx-64-ny-64-nz-64-nodes-4-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
