...
for i in $(seq 1 10); do
    srun --nodes=1 \
        ...
        --dirname /lus/bnchlu1/shanks/EPIC/data/rt-64x64x64/late-time \
        --ncbasename epic_rt_64x64x64_late \
        --niter 10 \
        --offset 1 \
        --nfiles 10 \
        --size-factor 1.5 \
        --csvfname "hotlum-cray-shmem-read-late-nx-64-ny-64-nz-64-nodes-1" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=1 \
            --ntasks=64 \
            ...
            --nfiles 10 \
            --size-factor 1.5 \
            --csvfname "hotlum-cray-$g-read-late-nx-64-ny-64-nz-64-nodes-1" \
            --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --nodes=1 \
                ...
                --nfiles 10 \
                --size-factor 1.5 \
                --csvfname "hotlum-cray-$g-read-late-nx-64-ny-64-nz-64-nodes-1-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
