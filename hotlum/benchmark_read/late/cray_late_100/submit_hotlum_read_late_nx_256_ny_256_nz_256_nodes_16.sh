...
for i in $(seq 1 5); do
    srun --nodes=16 \
        ...
        --dirname /lus/bnchlu1/shanks/EPIC/data/rt-256x256x256/late-time \
        --ncbasename epic_rt_256x256x256_late \
        --niter 100 \
        --offset 1 \
        --nfiles 10 \
        --size-factor 1.5 \
        --csvfname "hotlum-cray-shmem-read-late-nx-256-ny-256-nz-256-nodes-16" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=16 \
            --ntasks=1024 \
            ...
            --nfiles 10 \
            --size-factor 1.5 \
            --csvfname "hotlum-cray-$g-read-late-nx-256-ny-256-nz-256-nodes-16" \
            --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --nodes=16 \
                ...
                --nfiles 10 \
                --size-factor 1.5 \
                --csvfname "hotlum-cray-$g-read-late-nx-256-ny-256-nz-256-nodes-16-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
