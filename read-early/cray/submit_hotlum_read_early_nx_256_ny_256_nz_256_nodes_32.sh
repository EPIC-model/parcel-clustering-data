...
for i in $(seq 1 5); do
    srun --nodes=32 \
        ...
        --dirname /lus/bnchlu1/shanks/EPIC/data/rt-256x256x256/early-time \
        --ncbasename epic_rt_256x256x256_early \
        --niter 100 \
        --offset 1 \
        --nfiles 10 \
        --size-factor 2.0 \
        --csvfname "hotlum-cray-shmem-read-early-nx-256-ny-256-nz-256-nodes-32" \
        --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --nodes=32 \
            --ntasks=2048 \
            ...
            --nfiles 10 \
            --size-factor 2.0 \
            --csvfname "hotlum-cray-$g-read-early-nx-256-ny-256-nz-256-nodes-32" \
            --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --nodes=32 \
                ...
                --nfiles 10 \
                --size-factor 2.0 \
                --csvfname "hotlum-cray-$g-read-early-nx-256-ny-256-nz-256-nodes-32-subcomm" \
                --comm-type "$g" \
                --subcomm
        fi
    done
