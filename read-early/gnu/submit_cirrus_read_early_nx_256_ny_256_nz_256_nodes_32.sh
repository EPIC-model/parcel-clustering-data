...
for i in $(seq 1 5); do
    srun --kill-on-bad-exit \
         ...
         --nfiles 10 \
         --size-factor 2.0 \
         --csvfname "cirrus-gnu-shmem-read-early-nx-256-ny-256-nz-256-nodes-32" \
         --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --kill-on-bad-exit \
             --nodes=32 \
             ...
             --nfiles 10 \
             --size-factor 2.0 \
             --csvfname "cirrus-gnu-$g-read-early-nx-256-ny-256-nz-256-nodes-32" \
             --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --kill-on-bad-exit \
                 ...
                 --nfiles 10 \
                 --size-factor 2.0 \
                 --csvfname "cirrus-gnu-$g-read-early-nx-256-ny-256-nz-256-nodes-32-subcomm" \
                 --comm-type "$g" \
                 --subcomm
        fi
    done
