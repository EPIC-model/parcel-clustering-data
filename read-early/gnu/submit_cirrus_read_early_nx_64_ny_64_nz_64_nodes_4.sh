...
for i in $(seq 1 5); do
    srun --kill-on-bad-exit \
         ...
         --nfiles 10 \
         --size-factor 1.5 \
         --csvfname "cirrus-gnu-shmem-read-early-nx-64-ny-64-nz-64-nodes-4" \
         --comm-type "shmem"
    wait
    for g in "p2p" "rma"; do
        srun --kill-on-bad-exit \
             ...
             --nfiles 10 \
             --size-factor 1.5 \
             --csvfname "cirrus-gnu-$g-read-early-nx-64-ny-64-nz-64-nodes-4" \
             --comm-type "$g"
	wait
        if test "true" = "true"; then
            srun --kill-on-bad-exit \
                 ...
                 --nfiles 10 \
                 --size-factor 1.5 \
                 --csvfname "cirrus-gnu-$g-read-early-nx-64-ny-64-nz-64-nodes-4-subcomm" \
                 --comm-type "$g" \
                 --subcomm
	    wait
        fi
