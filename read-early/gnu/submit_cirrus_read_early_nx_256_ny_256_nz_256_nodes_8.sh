...
#         --nfiles 10 \
#         --size-factor 1.5 \
#         --csvfname "cirrus-gnu-shmem-read-early-nx-256-ny-256-nz-256-nodes-8" \
#         --comm-type "shmem"
    for g in "rma"; do #"p2p" "rma"; do
        srun --kill-on-bad-exit \
             --nodes=8 \
             --ntasks=288 \
             --unbuffered \
             ...
             --nfiles 10 \
             --size-factor 1.5 \
             --csvfname "cirrus-gnu-$g-read-early-nx-256-ny-256-nz-256-nodes-8" \
             --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --kill-on-bad-exit \
                 ...
                 --nfiles 10 \
                 --size-factor 1.5 \
                 --csvfname "cirrus-gnu-$g-read-early-nx-256-ny-256-nz-256-nodes-8-subcomm" \
                 --comm-type "$g" \
                 --subcomm
        fi
    done
