...
for i in $(seq 1 5); do
    srun --kill-on-bad-exit \
         ...
         --nfiles 10 \
         --size-factor 1.5 \
         --csvfname "cirrus-gnu-shmem-read-early-nx-128-ny-128-nz-128-nodes-16" \
         --comm-type "shmem"
    for g in "p2p" "rma"; do
        srun --kill-on-bad-exit \
             --nodes=16 \
             ...
             --nfiles 10 \
             --size-factor 1.5 \
             --csvfname "cirrus-gnu-$g-read-early-nx-128-ny-128-nz-128-nodes-16" \
             --comm-type "$g"
        ...
        if test "true" = "true"; then
            srun --kill-on-bad-exit \
                 ...
                 --nfiles 10 \
                 --size-factor 1.5 \
                 --csvfname "cirrus-gnu-$g-read-early-nx-128-ny-128-nz-128-nodes-16-subcomm" \
                 --comm-type "$g" \
                 --subcomm
        fi
    done
